    ...
    def __call__(self, *modifies):
        ...
        for (peripheral, register), field_values in modifies.items():
            match field_values:
                ...
                case _:
                    with self.Meta.enter(self.macro, '(', ');'):
                        for just_lhs, just_rhs in justify(
                            ...
                            self.Meta.line(f'{just_lhs}, {just_rhs},')
...
CMSIS_SET   = lambda Meta: CMSIS_MODIFY('CMSIS_SET'  , Meta)
CMSIS_WRITE = lambda Meta: CMSIS_MODIFY('CMSIS_WRITE', Meta)
...
def CMSIS_SPINLOCK(Meta, *spinlocks):
