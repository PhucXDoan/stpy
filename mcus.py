...
# in that it's a falsy singleton. It's used for parameterizatoin to
# indicate that a particular database entry hasn't been assigned a value yet;
...
# This is useful during parameterization when we want to
# brute-force the MCU's configuration.
#
    ...
    def __init__(self, name):
        ...
        exec(
            pathlib.Path(__file__)
                .parent
                .joinpath(f'databases/{self.name}.py')
                .read_text(),
            database_globals,
            {},
        )
        ...
        for proper_key, entry in database_globals['SCHEMA'].items():
            ...
            # It's convenient to have some database entries
            # to go by multiple keys, so we'll need to do
            # some key mapping in order to do that.

            self.translation |= {
                given_key : proper_key
                for given_key in (proper_key, *entry.pop('pseudokeys', ()))
            }
            ...
            if constraint is not None and not isinstance(constraint, Constraint):
                raise ValueError(
                    f'Database entry {repr(proper_key)} for MCU {repr(self.name)} '
                    f'has unknown constraint: {repr(constraint)}.'
                )

            self.database[proper_key].constraint = constraint
        ...
        for entry in csv.DictReader(
            pathlib.Path(__file__)
                .parent
                .joinpath(f'databases/{self.name}.csv')
                .read_text()
                .splitlines()
        ):
            ...
            match entry['Type']:
                ...
                case 'I/O':
                    ...
                    # Some GPIO names are suffixed with additional
                    # things, so we need to format it slightly so
                    # that we just get the port letter and pin number.
                    # e.g:
                    # >
                    ...
                    for code in range(16):
                        ...
                        for alternate_function in entry[f'AF{code}'].split('/'):
                            ...
                            assert key not in self.gpio_afsel_table

                            self.gpio_afsel_table[key] = code
                ...
                # Unknown GPIO type in the CSV; doesn't
                # neccessarily mean an error though.
